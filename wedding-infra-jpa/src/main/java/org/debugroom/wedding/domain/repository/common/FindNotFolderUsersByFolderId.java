...
public class FindNotFolderUsersByFolderId implements Specification<User>{
	...
	public Predicate toPredicate(Root<User> root, 
		...
		Join<User, UserRelatedFolder> subQueryJoinUserRelatedFolder = subQueryRoot.join(User_.userRelatedFolders);
		Join<UserRelatedFolder, Folder> subQueryJoinFolder = subQueryJoinUserRelatedFolder.join(UserRelatedFolder_.folder);
		Predicate subQueryPredicate = criteriaBuilder.equal(subQueryJoinFolder.get(Folder_.folderId), folderId);
